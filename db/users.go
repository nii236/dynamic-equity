...
var (
	userColumns               = []string{"id", "email", "username"}
	userColumnsWithoutDefault = []string{"email", "username"}
...
func (o *User) Chunks(mods ...qm.QueryMod) chunkQuery {
...
// PiePies retrieves all the py's Pies with an executor via id column.
func (o *User) PiePies(mods ...qm.QueryMod) pieQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"pies_users\" on \"pies\".\"id\" = \"pies_users\".\"pie_id\""),
		qm.Where("\"pies_users\".\"user_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"pies\".*"})
	}
...
func (userL) LoadChunks(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`chunks`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadPiePies allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadPiePies(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.Select("\"pies\".*, \"a\".\"user_id\""),
		qm.From("\"pies\""),
		qm.InnerJoin("\"pies_users\" as \"a\" on \"pies\".\"id\" = \"a\".\"pie_id\""),
	...
	results, err := query.Query(e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load pies")
	}
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Currency, &one.NonCashMultiplier, &one.CashMultiplier, &one.CommissionRatePercent, &one.RoyaltyRatePercent, &one.FindersFeePercent, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for pies")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice pies")
		}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on pies")
	}
	if err = results.Err(); err != nil {
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.PiePies = append(local.R.PiePies, foreign)
				if foreign.R == nil {
					foreign.R = &pieR{}
				}
...
func (userL) LoadRoles(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Role, &one.Action, &one.Target, &localJoinCol)
		if err != nil {
...
func (o *User) AddChunks(exec boil.Executor, insert bool, related ...*Chunk) error {
	var err error
...
// AddPiePies adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.PiePies.
// Sets related.R.Users appropriately.
func (o *User) AddPiePies(exec boil.Executor, insert bool, related ...*Pie) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into \"pies_users\" (\"user_id\", \"pie_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
	...
	if o.R == nil {
		...
		o.R.PiePies = append(o.R.PiePies, related...)
	}
...
// SetPiePies removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Users's PiePies accordingly.
// Replaces o.R.PiePies with related.
// Sets related.R.Users's PiePies accordingly.
func (o *User) SetPiePies(exec boil.Executor, insert bool, related ...*Pie) error {
	query := "delete from \"pies_users\" where \"user_id\" = $1"
	...
	removePiePiesFromUsersSlice(o, related)
	if o.R != nil {
		o.R.PiePies = nil
	}
	return o.AddPiePies(exec, insert, related...)
}

// RemovePiePies relationships from objects passed in.
// Removes related items from R.PiePies (uses pointer comparison, removal does not keep order)
// Sets related.R.Users.
func (o *User) RemovePiePies(exec boil.Executor, related ...*Pie) error {
	var err error
	query := fmt.Sprintf(
		"delete from \"pies_users\" where \"user_id\" = $1 and \"pie_id\" in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.PiePies {
			...
			ln := len(o.R.PiePies)
			if ln > 1 && i < ln-1 {
				o.R.PiePies[i] = o.R.PiePies[ln-1]
			}
			o.R.PiePies = o.R.PiePies[:ln-1]
			break
		}
	}
...
func removePiePiesFromUsersSlice(o *User, related []*Pie) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
