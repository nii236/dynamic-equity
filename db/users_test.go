...
func testUserToManyChunks(t *testing.T) {
	var err error
	...
	slice := UserSlice{&a}
	if err = a.L.LoadChunks(tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.Chunks = nil
	if err = a.L.LoadChunks(tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Chunks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyPiePies(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, pieDBTypes, false, pieColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, pieDBTypes, false, pieColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	_, err = tx.Exec("insert into \"pies_users\" (\"user_id\", \"pie_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"pies_users\" (\"user_id\", \"pie_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}

	check, err := a.PiePies().All(tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadPiePies(tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PiePies); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.PiePies = nil
	if err = a.L.LoadPiePies(tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PiePies); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyRoles(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"users_roles\" (\"user_id\", \"role_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"users_roles\" (\"user_id\", \"role_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}

	check, err := a.Roles().All(tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadRoles(tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.Roles = nil
	if err = a.L.LoadRoles(tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Roles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpChunks(t *testing.T) {
	var err error
...
func testUserToManyAddOpPiePies(t *testing.T) {
	var err error
	...
	foreigners := []*Pie{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, pieDBTypes, false, strmangle.SetComplement(piePrimaryKeyColumns, pieColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Pie{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddPiePies(tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.PiePies[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.PiePies[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.PiePies().Count(tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManySetOpPiePies(t *testing.T) {
	var err error
...
func testUserToManyRemoveOpPiePies(t *testing.T) {
	var err error
	...
	err = a.AddPiePies(tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemovePiePies(tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManyRemoveOpRoles(t *testing.T) {
	...
	err = a.AddRoles(tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveRoles(tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Email`: `text`, `Username`: `text`}
	_           = bytes.MinRead
)
